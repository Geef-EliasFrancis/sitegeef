...
const responsiveContract = [
  ...
  ['submenu público da reunião possui início', publicSiteHeaderSource.includes('{ href: "/reuniao-publica", label: "Início" }') && publicSiteHeaderSource.includes('{ href: "/reuniao-publica/programacao", label: "Programação" }') && publicSiteHeaderSource.includes('if (href === "/reuniao-publica") return pathname === href')],
  ['programação possui hero compacto em duas linhas', fs.readFileSync('app/programacao/page.tsx', 'utf8').includes('showBrandKicker={false}') && fs.readFileSync('app/programacao/page.tsx', 'utf8').includes('compactHero') && fs.readFileSync('components/content-page.tsx', 'utf8').includes('compactHero?: boolean')],
