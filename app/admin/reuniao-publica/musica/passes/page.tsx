...
export default async function MusicaPassesAdminPage() {
  ...
  return <AdminModuleGate permission="pode_publicar" profiles={["diretoria", "secretaria", "comunicacao"]} redirectPath="/admin/reuniao-publica/musica/passes" title="Passes">
    <div className="area-page"><div className="admin-page-header"><h1 className="admin-page-title">Passes</h1></div>
      <section className="area-section"><div className="admin-card table-surface"><form action={addPasse} encType="multipart/form-data" className="musica-passes-admin-form"><input name="titulo" className="profile-form-input" placeholder="Título do áudio" required /><label className="musica-passes-file-field"><span>Selecionar MP3</span><input name="audio" type="file" accept="audio/mpeg,.mp3" required /></label><button className="admin-btn admin-btn-primary" title="Adicionar áudio"><IconPlus size={18} /></button></form></div></section>
