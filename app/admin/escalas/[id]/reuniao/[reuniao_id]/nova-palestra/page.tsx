...
async function handleSubmit(formData: FormData, escalaId: string, reuniaoId: string) {
  'use server';
  const result = await createEscalaPalestra({ reuniaoId, palestranteId: String(formData.get('palestrante_id') || ''), temaId: String(formData.get('tema_id') || '') || undefined, temaLivre: String(formData.get('tema_livre') || ''), tipoPalestra: String(formData.get('tipo_palestra') || '') });
