...
- Um job de `npm_test` deste repositório foi validado na homelab com 74 testes
  aprovados.
- **Ainda não existe uma ponte MCP ou comando do GEEF que permita ao Codex
  enviar jobs ao coordenador.** Portanto, o Codex não deve afirmar que usou a
  homelab sem evidência de um job concluído.
...
Criar uma ponte MCP local ao projeto, com três ferramentas estritamente
tipadas:
...
A ponte deverá montar a credencial por caminho de arquivo, aceitar apenas o
repositório `JeanMRocha/sitegeef`, validar SHA/ref e não disponibilizar um
executor genérico. A seleção `auto` e o fallback VPS só devem ser expostos
após validação fim a fim dessa ponte.
...
4. Implementar e testar a ponte com um commit de teste inofensivo antes de
   habilitá-la ao Codex.
5. Registrar evidência do job (SHA, tipo, executor, status e resumo dos logs)
